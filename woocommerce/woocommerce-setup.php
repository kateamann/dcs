 ...
 * DCS.
 *
 * This file adds the required WooCommerce setup functions to the DCS Theme.
 *
 * @package DCS
 * @author  StudioPress
...
add_action( 'wp_enqueue_scripts', 'dcs_products_match_height', 99 );
/**
 * Prints an inline script to the footer to keep products the same height.
 *
 * @since 2.3.0
 */
function dcs_products_match_height() {
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'dcs_woocommerce_breakpoint' );
/**
 ...
 */
function dcs_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'dcs_default_products_per_page' );
/**
 ...
 */
function dcs_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'dcs_woocommerce_pagination' );
/**
 ...
 */
function dcs_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'dcs' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'dcs' ) );

	return $args;

}

add_action( 'after_switch_theme', 'dcs_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 2.3.0
 */
function dcs_woocommerce_image_dimensions_after_theme_setup() {
	...
	dcs_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'dcs_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function dcs_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function dcs_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'dcs_gallery_image_thumbnail' );
/**
 ...
 */
function dcs_gallery_image_thumbnail( $size ) {
